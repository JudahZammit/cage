...
class TimeStep(layers.Layer):
  def __init__(self,up,filters,shape,trainable = True):
    super(TimeStep,self).__init__(trainable = trainable,autocast = False)
    ...
    for _ in range(BUCKETS):
      self.down_0.append(Down(filters))
      self.s_0.append(Smooth(filters))
  ...
  def call(self,inputs,time):
    ...
    right_partition = []
    for _ in range(BUCKETS):
      right_partition.append([])

    for i in range(len(time)):
      right_partition[int(time[i])].append(right[i])
    ...
    if self.up != None:
      ...
      lower_partition = []
      for _ in range(BUCKETS):
        lower_partition.append([])

      for i in range(len(time)):
        lower_partition[int(time[i])].append(lower[i])
      
      concat_partition = []
      for _ in range(BUCKETS):
        concat_partition.append([])

      for i in range(BUCKETS):
        for j in range(len(right_partition[i])):
          concat_partition[i].append(tf.concat(
                                    [lower_partition[i][j],right_partition[i][j]]
                                    ,axis = -1))


      down_partition = []  
      for _ in range(BUCKETS):
        down_partition.append([])

      for i in range(BUCKETS):
        collect = []
        for j in range(len(concat_partition[i])):
          collect.append(concat_partition[i][j])
         
        if len(collect) != 0: 
          down = self.down_0[i](tf.stack(collect,axis = 0))
          
          for d in range(len(collect)):
            down_partition[i].append(down[d])

      right_partition = down_partition
     
    out_partition = []  
    for _ in range(BUCKETS):
      out_partition.append([])

    for i in range(BUCKETS):
      collect = []
      for j in range(len(right_partition[i])):
        collect.append(right_partition[i][j])
          
      if len(collect) != 0:
        smooth = self.s_0[i](tf.stack(collect,axis = 0))
  
        for s in range(len(collect)):
          out_partition[i].append(smooth[s])
    
     
    out = []
    counter = np.zeros((BUCKETS))
    for t in time:
      out.append(out_partition[int(t)][int(counter[int(t)])])
      counter[int(t)] += 1
...
class DecoderBlock(layers.Layer):
  def __init__(self,in_filters,out_filters):
    ...
    self.l0 = DecoderTransposeX2Block(in_filters,
            use_batchnorm = True,out = 'relu')
    self.l1 = LightSmooth(in_filters)
    self.l2 = Down(out_filters)
    self.l3 = Smooth(out_filters)
...
def define_discriminator(in_shape=(SHAPE,SHAPE,1), n_classes=BUCKETS):
        # label input
        in_label = tfkl.Input(shape=())
        # embedding for categorical input
        li = tfkl.Embedding(n_classes, 10*BUCKETS)(in_label)
        # scale up to image dimensions with linear activation
        n_nodes = in_shape[0] * in_shape[1]
        li = tfkl.Dense(n_nodes)(li)
        # reshape to additional channel
        li = tf.reshape(li,[BS*BUCKETS,SHAPE,SHAPE,1])
        # image input
        in_image = tfkl.Input(shape=in_shape)
        # concat label as a channel
        merge = tf.concat([in_image, li],axis = -1) 
        backbone = tfk.applications.MobileNetV2(input_shape = (SHAPE,SHAPE,2),
                                          include_top = True,
                                          weights = None,
                                          classes = 1,
                                          classifier_activation = 'sigmoid')
        out_layer = backbone(merge)
        model = tfk.models.Model([in_image, in_label], out_layer)
